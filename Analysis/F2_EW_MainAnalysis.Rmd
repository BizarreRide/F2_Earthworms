...
env.chosen <- cbind(resid(M0),data[,c("anc", "mc", "pH", "cn", "clay", "ats1", "ats2", "hum1", "dgN", "dgO","dsamcam")])

melted_a <- melt(env.chosen, id.vars="resid(M0)")

ggplot(melted_a, aes(value, y="resid(M0)")) +
  geom_point() +
  facet_grid(. ~ variable)

par(mfrow=c(4,3))
for (i in 1:12) {
  plot(env.chosen[,i],env.chosen[,1], xlab=colnames(env.chosen)[i])
}



colnames(env.chosen)[i])
dev.off()


plot(data$age_class,resid(M0))
plot(data$field,resid(M0))
plot(env.chosen[,1],resid(M0))
plot(env.chosen[,2],resid(M0))
plot(env.chosen[,3],resid(M0))
plot(env.chosen[,4],resid(M0))
plot(env.chosen[,5],resid(M0))
plot(env.chosen[,6],resid(M0))



env.chosen <- data[,c("anc", "mc", "pH", "cn", "clay", "ats1", "ats2", "hum1", "dgN", "dgO","dsamcam")]
op <- par(mfrow=c(1,1), pty="s")
pairs(cbind(resid(M1),env.chosen), lower.panel=panel.cor, upper.panel=panel.smooth, method="pearson", diag.panel=panel.hist, main= "Bivariate Plots with Histograms and smooth curves\n(pearson)")
par(op)
...
plot(data$age_class, resid(M1))


M2 <- lmer(anc ~ age_class + pH + (1|field.ID), data)
summary(M2)
