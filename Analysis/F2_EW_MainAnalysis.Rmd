...
anc.dredge <- dredge(M2)
head(anc.dredge,10)

anc.best <- glmmadmb(anc ~ age_class + samcam + I(scl.mc^2) + (1|field.ID) + offset(log(area)) ,data=data,family="poisson")
summary(anc.best)
...
abline(h=0, lty=2)
plot(E ~ data$mc, ylab="Pearson Residuals", xlab="Age Class")
abline(h=0, lty=2)
````
...
ggplot(anc.pred, aes(x = age_class, y = exp(fit), ymin = exp(lwr), ymax = exp(upr))) + 
  ...
  geom_errorbar(position = position_dodge(1),col="red") + 
  facet_grid(.~samcam) +
  mytheme
...
anc.td = expand.grid(age_class=unique(data$age_class),
                     samcam = unique(data$samcam),               
                     scl.mc = seq(min(data$scl.mc),max(data$scl.mc), by=0.2),
                     area = 1)

anc.pred <- cbind(anc.td, predict(anc.best, newdata = anc.td, interval = "confidence")) 

ggplot(anc.pred, aes(x = scl.mc, y = exp(fit), ymin = exp(lwr), ymax = exp(upr))) + 
