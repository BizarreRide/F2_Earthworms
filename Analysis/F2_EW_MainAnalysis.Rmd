...
ggplot(data.rf, aes(x=age_class, y=4*abc.mean, fill=sfg)) +  
  ...
  geom_errorbar(aes(ymin=4*(abc.mean-abc.se), ymax=4*(abc.mean+abc.se)), position=position_dodge(0.9),width=0.2) +
  mytheme
...
data2.rf <- cbind(data2.rf.melted,data2.rf.melted2[,c(2:6)])

# Barplots
ggplot(data2.rf, aes(x=age_class, y=abc.mean, fill=sfg)) +  
  geom_bar(stat="identity", position="dodge") + 
  geom_errorbar(aes(ymin=abc.mean-abc.se, ymax=abc.mean+abc.se), position=position_dodge(0.9),width=0.2) +
  mytheme
