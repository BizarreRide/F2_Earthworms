...
```{r}
M1 <- glmer.nb(anc ~ age_class + I(ats1^2) + I(hum1^2) + (1|samcam), data2)
summary(M1)
...
```{r glmm}
M2 <- lmer(anc ~ scl.pH + (1|field.ID), data)
...
X   <- model.matrix(~ scl.pH, data)
Fit <- X %*% fixef(M2)

par(mfrow = c(1, 2), mar = c(5,5,2,2))
plot(x = data$scl.pH, 
     y = data$anc,
     xlab = "pH value (scaled)",
     ylab = "Anecic Abundance",
     cex.lab = 1.5, pch = 1)
lines(data$pH, Fit, lwd = 5, col="red")
...
*****************************************************************************************************************************
M2 <- glmmadmb(anc ~ age_class*scl.dsamcam + scl.mc + scl.pH*scl.cn + I(scl.ats1^2) +I(scl.hum1^2) + (1|field.ID),data=data,family="poisson")
summary(M2)

E1 <- resid(M2, type="pearson")
F1 <- fitted(M2)

env1 <- cbind(E1, F1, data[,c("anc", "age_class", "dsamcam")])
env0 <- cbind(E1, F1,data[,c("mc", "pH", "cn", "clay", "ats1", "ats2", "hum1","dgO")])
...
# Overdispersion
N <- nrow(data)
p <- length(coef(M2))+1 # The Plus "+1" used for determining the number of parameters(p) is due to the "k" of negative 
                        # binomial distribution
Dispersion <- sum(E1^2)/(N-p)
Dispersion
