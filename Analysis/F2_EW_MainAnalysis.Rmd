...
# Variability within sites
boxplot(endo2~field.ID, data, col="grey", main="Variability within sites", xlab="sites orderd in decreasing age", ylab="anecic earthworm abundance")

# variability within sampling campaigns
boxplot(endo2~samcam, data,col="grey", main="Variability within sampling campaigns", xlab="seasons:\n autumn2012, spring2013, autumn2013", ylab="anecic earthworm abundance")
...
# Model Formulation
endo.glob1 <- glmmadmb(endo ~ age_class + samcam + scl.mc + scl.pH*scl.cn + (1|field.ID) + offset(log(area)),data=data,family="nbinom")
endo.glob1 <- glmmadmb(endo ~ age_class + samcam + I(scl.mc^2) + scl.pH*scl.cn + (1|field.ID) + offset(log(area)),data=data,family="nbinom")
endo.glob1 <- glmmadmb(endo ~ age_class + samcam + I(scl.ats1^2) + scl.pH*scl.cn + (1|field.ID) + offset(log(area)),data=data,family="nbinom")
# offsset is used due to the personal advice by T. Onkelinx:
# You better use an offset if you want to express the model in terms of m². Just add offset(log(0.25)) to the model. 
...
# Multimodel averaging
endo.dredge3 <- dredge(endo.glob)
head(endo.dredge3,10)

# The best model with both glmer() and glmmadmb():
# endo.best <- glmer(endo ~ age_class*samcam + I(scl.ats1^2) + (1|field.ID) + offset(log(area)) ,data=data,family="poisson")
endo.best <- glmmadmb(endo ~ age_class + samcam + scl.mc + scl.pH + (1|field.ID) + offset(log(area)) ,data=data,family="nbinom")
endo.best <- glmer.nb(endo ~ age_class + samcam + scl.mc + scl.pH + (1|field.ID) + offset(log(area)) ,data=data)
summary(endo.best)
...
# Dataset with all variables IN the model
env1 <- cbind(E1, F1, data[,c("endo", "age_class", "samcam", "mc","pH")])
# covariates NOT in the model
env0 <- cbind(E1, F1,data[,c("ats1", "cn", "clay", "ats2", "hum1","dgO")])
